...
class ConsoleState(object):
    ...
    This class defines all members of the console required to run code through
    it. This can be used to evaluate scripts.
    """

    def __init__(self, game: 'app.Game') -> None:
        self.history = list()
        self.environ = {
            'game': game, 'console': self, 'print': self.print,
            'close': self.close, 'reset': self.reset, 'clear': self.clear
        }
    ...
    def print(self, *objects, sep='', end='\n', file=None) -> None:
        ...
        :param sep: The string used to separate the objects being printed
        :param end: The string to terminate the printed string with
        :param file: An IO object to write the string to
        """
        if file is not None:
    ...
    def evaluate(self, code_string) -> 'Any':
        ...
        This method does not attempt to do any kind of sand-boxing as that is
        not possible. This means that the user of the debug console is able to
        run any valid python program.
...
class ConsoleWindow(tk.Toplevel):
    ...
    def __init__(self, root: 'tk.Tk', state: 'ConsoleState', **kwargs) -> None:
        ...
        self._txtOutput = widgets.StaticTextArea(
            self._frame, yscrollcommand=self._scrollOutput.set
        )
        self._txtInput = widgets.CustomTextArea(
            self._frame, height=1, yscrollcommand=self._scrollInput.set
        )

        self._txtInput.bind("<Return>", self._on_enter)
        # Move to previous history line
        self._txtInput.bind("<Up>", self._arrow_up)
        # Move to next history line
        self._txtInput.bind("<Down>", self._arrow_down)

        def _key_code_d(event: 'tk.Event') -> None:
            if widgets.control(event) and not widgets.alt(event):
                self.close()
        self._txtInput.bind("<D>", _key_code_d)
    ...
    def _on_enter(self, event) -> 'Optional[str]':
        ...
        This method will evaluate the code in the input text box as python
        code, using the ConsoleState object bound to it to execute the code.
        ...
        # Otherwise, we need to display the text, reset the input box, run the
        ...
        value_add = "\n".join(
            ">>> {}".format(part) for part in value.split("\n")
        ) + "\n"
    ...
    def _arrow_up(self, _) -> 'Optional[str]':
        ...
        if y == 1:
            ...
            if hist_line != self._hist_line:
                ...
                self._txtInput.delete('1.0', 'end')
                self._txtInput.insert(
                    '1.0', self._state.history[self._hist_line]
                )
            return "break"
