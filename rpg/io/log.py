...
class Log(object):
    ...
    _level_lookup = {
        'debug': LogLevel.Debug, 'verbose': LogLevel.Verbose,
    ...
    def level(self, level: 'Optional[LogLevel]' = None) -> LogLevel:
        ...
        :param level: The logging level to set this log object to, or None to
                      leave it the same
    ...
    def echo(self, on: 'Optional[bool]' = None) -> bool:
        """Get if this Log is echoing output to stdout, optionally changing
        said behaviour.

        :param on: If echoing should be performed, otherwise None to leave
                   behavior the same
    ...
    def write(self, level: LogLevel, message: str, *args, **kwargs) -> bool:
        """Write a log statement with the given level if that level is at least
